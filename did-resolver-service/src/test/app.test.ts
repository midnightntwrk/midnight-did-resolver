...
describe("did-resolver-service app", () => {
  ...
  it("rejects query-level indexer endpoint overrides", async () => {
    const service = { resolve: vi.fn() } as unknown as ResolverService;
    const app = await createApp(service);
    const response = await app.inject({
      method: "GET",
      url: "/resolve/did%3Amidnight%3Adevnet%3Aabc?indexerUrl=https%3A%2F%2Fattacker.example%2Fgraphql",
    });
  ...
  it("rejects body-level indexer endpoint overrides", async () => {
    const service = { resolve: vi.fn() } as unknown as ResolverService;
    const app = await createApp(service);
    const response = await app.inject({
      method: "POST",
      ...
      payload: {
        ...
        indexerUrl: "https://attacker.example/graphql",
      },
    });
