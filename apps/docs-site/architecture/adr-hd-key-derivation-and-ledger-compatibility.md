...
|---|---|---|---|
| Seed validation | `SeedSchema`, `parseSeed`, `seedToBuffer` | `zod`, Node buffers | `packages/secret-storage/src/seed.ts` |
| Metadata key derivation | `deriveMetadataKey` | `@midnight-ntwrk/wallet-sdk-hd` (`HDWallet`, `Roles.Metadata`) | `packages/secret-storage/src/hd-derivation.ts` |
| HKDF expansion | `hkdfSync` with salt/info contract | Node `crypto` HKDF | `packages/secret-storage/src/hd-derivation.ts` |
| Curve-specific private key shaping | `deriveCurvePrivateFromSeed`, `normalizeP256Private` | curve logic + bigint math | `packages/secret-storage/src/hd-derivation.ts` |
| Key import + public key materialization | `importCurveKey` | Node crypto + Jubjub helpers | `packages/secret-storage/src/curve-support.ts` |
| Ledger representability checks | `isPublicJwkLedgerCompatible`, `assertPublicJwkLedgerCompatible` | `@midnight-ntwrk/ledger-v8` (`maxField`, field helpers) | `packages/secret-storage/src/curve-support.ts` |
| Retry for representable key | `FileSecretStore.deriveKeyFromSeed` (`candidate` loop) | `secret-storage` store logic | `packages/secret-storage/src/file-secret-store.ts` |
