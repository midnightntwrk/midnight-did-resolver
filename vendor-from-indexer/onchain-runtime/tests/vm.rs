#[cfg(test)]
mod tests {
    use midnight_onchain_runtime::base_crypto::fab::{Aligned, AlignedValue};
    use midnight_onchain_runtime::coin_structure::contract::Address;
    ...
    use midnight_onchain_runtime::transient_crypto::fab::{AlignmentExt, ValueReprAlignedValue};
    use midnight_onchain_runtime::transient_crypto::merkle_tree::{
        MerkleTree, MerkleTreeDigest, leaf_hash,
    };
    ...
    fn individual_vm_instructions() {
        ...
        let run_program = run_program::<InMemoryDB>;
    ...
    fn merkle_tree_flow() {
        ...
        fn root(value: VmValue<InMemoryDB>) -> MerkleTreeDigest {
            match &value.value {
                StateValue::BoundedMerkleTree(t) => t.root(),
                _ => unreachable!(),
            }
        }
    ...
    fn test_context_self() {
        ...
        let dummy_with_context = vec![
            vmval!(# [(AlignedValue::from(Address::default())), null, null]),
            vmval!(# [{}, {}, {}, {}, {}]),
            vmval!(null),
        ];
        assert_eq!(
            ...
            Ok((
                ...
                vec![read!(AlignedValue::from(Address::default()))]
            ))
        );
    }
}
