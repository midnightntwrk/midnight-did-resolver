...
pub trait DB: Default + Sync + Send + Debug + DummyArbitrary + 'static {
    ...
    ) -> Vec<(ArenaKey<Self::Hasher>, OnDiskObject<Self::Hasher>)>
    where
...
mod tests {
    ...
    fn test_bfs_get_nodes<D: DB<Hasher = DefaultHasher>>() {
        ...
        let keys: Vec<_> = kvs.clone().into_iter().map(|(k, _)| k).collect();
        let expected_keys: Vec<_> = [&n11, &n31, &n21, &n22, &n41, &n42, &n32, &n33, &n43, &n44]
            .map(|n| n.key.clone())
            .into_iter()
            .collect();
        ...
        let mut keys: Vec<_> = kvs.into_iter().map(|(k, _)| k).collect();
        ...
        let keys: Vec<_> = kvs.into_iter().map(|(k, _)| k).collect();
