...
use base_crypto::serialize::{NetworkId, deserialize, serialize};
...
impl<H: WellBehavedHasher> ToSql for Children<H> {
    fn to_sql(&self) -> Result<rusqlite::types::ToSqlOutput<'_>> {
        let mut buf = vec![];
        serialize(&self.0, &mut buf, NetworkId::Undeployed).unwrap();
        Ok(buf.into())
    }
}
...
impl<H: WellBehavedHasher> FromSql for Children<H> {
    fn column_result(value: rusqlite::types::ValueRef<'_>) -> rusqlite::types::FromSqlResult<Self> {
        Ok(Children(
            deserialize(value.as_bytes()?, NetworkId::Undeployed).unwrap(),
        ))
    }
}
...
mod tests {
    ...
    fn test_concurrent_access(mk_db: impl Fn() -> SqlDB) {
        ...
        let k: ArenaKey<_> = rng.gen();
        let v: OnDiskObject<_> = rng.gen();
        let mut jobs = vec![];
    ...
    fn env_override_sqlite_params() {
        ...
        env::set_var("MIDNIGHT_STORAGE_DB_SQL_JOURNAL_MODE", "DELETE");
        env::set_var("MIDNIGHT_STORAGE_DB_SQL_SYNCHRONOUS", "2");
        let path: tempfile::TempPath = tempfile::NamedTempFile::new().unwrap().into_temp_path();
        let db = SqlDB::exclusive_file(&path);
        let journal_mode: String = query_pragma(&db, "journal_mode");
