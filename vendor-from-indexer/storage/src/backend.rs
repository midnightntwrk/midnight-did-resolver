...
use base_crypto::serialize::{Deserializable, Serializable, Versioned};
...
impl<D: DB> StorageBackend<D> {
    ...
    pub(crate) fn get(&mut self, key: &ArenaKey<D::Hasher>) -> Option<&OnDiskObject<D::Hasher>> {
        ...
        if self.peek_from_memory(key).is_some() {
            ...
            let value = match value {
                ...
                | CacheValue::CreateAndDelete { .. } => value,
            };
    ...
    pub(crate) fn cache(
        ...
        if self.peek_from_memory(&key).is_some() {
            ...
            let value = match value {
                ...
                | CacheValue::ReadAndUpdate { .. }
                | CacheValue::Create { .. }
                | CacheValue::CreateAndUpdate { .. } => value,
    ...
    pub fn gc(&mut self) {
        ...
        let root_keys: HashSet<_> = self
            ...
            .collect();
        let mut unreachable_keys: Vec<_> = db_unreachable_keys
            .into_iter()
    ...
    fn remove_from_memory(&mut self, key: &ArenaKey<D::Hasher>) -> CacheValue<D::Hasher> {
        if self.write_cache.peek(key).is_some() {
            self.write_cache.remove(key).unwrap().1
        } else if self.read_cache.peek(key).is_some() {
            self.read_cache.remove(key).unwrap().1
        } else {
            panic!("key must be in memory");
        }
    }
    ...
    fn update_counts(&mut self, keys: &[ArenaKey<D::Hasher>], delta: Delta) {
        for key in keys {
            if self.peek_from_memory(key).is_none() {
                ...
                let result = match self.remove_from_memory(key) {
                    CacheValue::Read { obj } => {
                        let obj = obj.apply_delta(delta);
                        Some(CacheValue::ReadAndUpdate { obj, delta })
                    }
                    CacheValue::Update {
                        delta: old_delta,
                        obj,
                        ...
                        if let Some(delta) = delta.combine(old_delta) {
                            Some(CacheValue::Update { delta, obj })
                        } else {
                            None
                        }
                    }
                    CacheValue::ReadAndUpdate {
                        ...
                        if let Some(delta) = delta.combine(old_delta) {
                            ...
                            Some(CacheValue::Read { obj })
                        }
                    }
                    CacheValue::Create { obj } => {
                        let obj = obj.apply_delta(delta);
                        Some(CacheValue::CreateAndUpdate { obj, delta })
                    }
                    CacheValue::CreateAndUpdate {
                        obj,
                        delta: old_delta,
                    ...
                    CacheValue::CreateAndDelete {
                        ...
                        let obj = obj.apply_delta(delta);
                        if let Some(delta) = delta.combine(old_delta) {
                            Some(CacheValue::CreateAndDelete { obj, delta })
                        } else {
    ...
    fn cache_insert_new_key(&mut self, key: ArenaKey<D::Hasher>, value: CacheValue<D::Hasher>) {
        ...
        if value.is_pending() {
            ...
            assert!(!v.is_pending(), "read cache shouldn't contain writes");
        }
    }
...
pub struct OnDiskObject<H: WellBehavedHasher> {
    ...
    pub(crate) children: Vec<ArenaKey<H>>,
}

impl<H: WellBehavedHasher> Versioned for OnDiskObject<H> {
    const VERSION: Option<base_crypto::serialize::Version> = None;
}
...
impl<H: WellBehavedHasher> Serializable for OnDiskObject<H> {
    fn unversioned_serialize<W: std::io::Write>(
        ...
        <Vec<u8> as Serializable>::serialize(&value.data, writer)?;
        <u32 as Serializable>::serialize(&value.ref_count, writer)?;
        <Vec<ArenaKey<H>> as Serializable>::serialize(&value.children, writer)?;
        Ok(())
    }

    fn unversioned_serialized_size(value: &Self) -> usize {
        <Vec<u8> as Serializable>::serialized_size(&value.data)
            + <u32 as Serializable>::serialized_size(&value.ref_count)
            + <Vec<ArenaKey<H>> as Serializable>::serialized_size(&value.children)
    }
}
...
impl<H: WellBehavedHasher> Deserializable for OnDiskObject<H> {
    fn versioned_deserialize<R: std::io::Read>(
        reader: &mut R,
        _version: Option<&base_crypto::serialize::Version>,
        recursion_depth: u32,
    ) -> Result<Self, std::io::Error> {
        let data = <Vec<u8> as Deserializable>::deserialize(reader, recursion_depth)?;
        let ref_count = <u32 as Deserializable>::deserialize(reader, recursion_depth)?;
        let children = <Vec<ArenaKey<H>> as Deserializable>::deserialize(reader, recursion_depth)?;
        Ok(OnDiskObject {
            data,
            ref_count,
            children,
...
impl<H: WellBehavedHasher> Distribution<OnDiskObject<H>> for Standard {
    ...
    fn sample<R: rand::prelude::Rng + ?Sized>(&self, rng: &mut R) -> OnDiskObject<H> {
        ...
        fn rand_vec<T, R: rand::prelude::Rng + ?Sized>(rng: &mut R) -> Vec<T>
        where
...
pub(crate) mod raw_node {
    ...
    impl<H: WellBehavedHasher> RawNode<H> {
        pub(crate) fn new(key: &[u8], ref_count: u32, children: Vec<&RawNode<H>>) -> Self {
            let data = key.to_vec();
...
#[cfg(test)]
mod tests {
    use crate::serialize::{self, Versioned};
    ...
    fn in_database_repr<D: DB, T: Storable<D>>(val: T) -> (ArenaKey<D::Hasher>, Vec<u8>) {
        let mut bytes: Vec<u8> = Vec::new();
        Storable::to_binary_repr(&val, &mut bytes).expect("Failed to serialize to 'Vec<u8>'!");
    ...
    #[derive(Versioned, Debug, Storable)]
    #[derive_where(Clone)]
    #[storable(db = D)]
    struct LabeledNode<D: DB> {
        label: u32,
        children: Vec<Sp<Self, D>>,
    }
    ...
    fn test_pre_fetch<D: DB>() {
        ...
        let test = |cache_size: usize| {
            ...
            backend.get(&n11.key);
            let lru_keys: Vec<_> = backend.read_cache.iter().map(|(k, _)| k.clone()).collect();
            let mut expected_keys: Vec<_> = [&n11, &n21, &n22, &n31, &n32, &n33, &n41]
                .map(|n| n.key.clone())
                .into_iter()
                .collect();
