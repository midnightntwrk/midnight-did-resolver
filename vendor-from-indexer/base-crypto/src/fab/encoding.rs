...
#[derive(Clone, Default, PartialEq, Eq, Hash, PartialOrd, Ord, Versioned)]
...
#[derive(Clone, PartialEq, Eq, Hash, PartialOrd, Ord, Versioned)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(
    feature = "serde",
    ...
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> AlignmentSegment {
        ...
        match discriminant {
            0 => {
                ...
                for _ in 0..size {
                    options.push(rng.gen())
                }
                AlignmentSegment::Option(options)
            }
            _ => AlignmentSegment::Atom(rng.gen()),
        }
    }
}
...
impl Distribution<AlignedValue> for Standard {
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> AlignedValue {
        let alignment: Alignment = rng.gen();
...
#[derive(Clone)]
#[cfg_attr(feature = "serde", derive(Serialize))]
#[cfg_attr(feature = "serde", serde(into = "Value"))]
...
#[cfg_attr(feature = "serde", serde(transparent))]
#[cfg_attr(any(test, feature = "fake"), derive(Dummy))]
pub struct ValueAtom(#[cfg_attr(feature = "serde", serde(with = "serde_bytes"))] pub Vec<u8>);
...
/// primitive data type.
#[derive(Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord, Versioned)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
...
impl BinaryHashRepr for AlignmentAtom {
    fn binary_repr<W: MemWrite<u8>>(&self, writer: &mut W) {
        Serializable::unversioned_serialize(self, &mut IoWrite(writer)).ok();
    }
    fn binary_len(&self) -> usize {
        Serializable::unversioned_serialized_size(self)
    }
}
