...
impl MidnightDataProvider {
    ...
    async fn get_or_fetch(&self, name: &str) -> io::Result<BufReader<File>> {
        ...
        let mut file = OpenOptions::new()
            ...
            .truncate(true)
            .open(&path)?;
        if let Err(e) = self.fetch_data_to(name, expected_hash, &mut file).await {
            ...
            let res = match reqwest::Client::new().get(url.clone()).send().await {
                ...
                Err(e) => {
                    match &self.output_mode {
                        #[cfg(feature = "cli")]
                        OutputMode::Cli(pb) => pb.println(format!("{e}. {retry_msg}"))?,
                        OutputMode::Log => warn!("{e}. {retry_msg}"),
                    }
                    continue;
                }
            };
