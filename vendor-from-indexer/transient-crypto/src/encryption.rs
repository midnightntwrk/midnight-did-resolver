...
impl Versioned for PublicKey {
    const VERSION: Option<serialize::Version> = Some(Version { major: 3, minor: 0 });
}
    ...
    fn versioned_deserialize<R: std::io::Read>(
        ...
        match version {
            Some(Version { major: 3, minor: 0 }) => Ok(Self(
                <EmbeddedGroupAffine as Deserializable>::deserialize(reader, recursion_depth)?,
            )),
...
impl Deserializable for SecretKey {
    fn versioned_deserialize<R: std::io::Read>(
        ...
        match version {
            Some(Version { major: 3, minor: 0 }) => Ok(Self(
                <EmbeddedFr as Deserializable>::deserialize(reader, recursion_depth)?,
            )),
            _ => Err(Self::deserialization_error(
                version,
                "Unrecognized version.".to_string(),
            )),
        }
    }
}
...
#[derive(Clone, Debug, Versioned, Serializable)]
