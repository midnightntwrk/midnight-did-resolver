...
mod proof_tests {
    ...
    use midnight_transient_crypto::proofs::{
        IrSource, KeyLocation, PARAMS_VERIFIER, ParamsProver, ParamsProverProvider, Preprocessed,
        ProofPreimage, ProverKey, Resolver, VerifierKey,
    };
    ...
    async fn test_keygen_and_serialize_eq() {
        ...
        Serializable::serialize(&vk_kzg1, &mut bytes).unwrap();
        let vk_kzg3: VerifierKey =
            serialize::deserialize(&mut &bytes[..], serialize::NetworkId::Undeployed).unwrap();
        assert_eq!(&vk_kzg1, &vk_kzg3);
    }

    #[cfg(feature = "proptest")]
    randomised_serialization_test!(ProverKey);
    #[cfg(feature = "proptest")]
