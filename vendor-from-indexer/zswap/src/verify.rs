...
#[cfg(feature = "proof-verifying")]
use transient_crypto::proofs::{PARAMS_VERIFIER, ParamsVerifier, VerifierKey};
...
lazy_static! {
    pub static ref OUTPUT_VK: VerifierKey =
        Deserializable::deserialize(&mut OUTPUT_VK_RAW.to_vec().as_slice(), 0)
            .expect("Zswap Output VK should be valid");
    pub static ref SPEND_VK: VerifierKey =
        Deserializable::deserialize(&mut SPEND_VK_RAW.to_vec().as_slice(), 0)
            .expect("Zswap Spend VK should be valid");
    pub static ref SIGN_VK: VerifierKey =
        Deserializable::deserialize(&mut SIGN_VK_RAW.to_vec().as_slice(), 0)
    ...
    pub fn well_formed(&self, _segment: u16) -> Result<(), MalformedOffer> {
        if let (Some(address), Some(ciphertext)) = (self.contract_address, &self.ciphertext) {
            return Err(MalformedOffer::ContractSentCiphertext {
                address,
                ciphertext: Box::new(ciphertext.clone()),
            });
        }
        Ok(())
    }
...
fn offer_well_formed_common<P: Ord>(
    offer: &Offer<P>,
    segment: u16,
    ...
    if !offer.inputs.iter().is_sorted()
        ...
        || !offer.deltas.windows(2).all(|slice| slice[0].0 < slice[1].0)
    {
    ...
    let deltas_com = offer
        ...
        .map(|(type_, value)| {
            Pedersen::commit(
                &(*type_, segment),
                &<EmbeddedFr as From<i128>>::from(*value),
                &0u64.into(),
            )
        })
        .fold(com_unit, Add::add);
