...
use base_crypto::serialize::Serializable;
...
impl Input<ProofPreimage> {
    ...
    pub fn new_contract_owned<
        A: Debug + Clone + Sync + Send + Serializable + Deserializable,
    ...
    >(
        ...
        Self::new_from_secret_key::<A, R, D>(
            rng,
            coin,
            segment,
    ...
    pub(crate) fn new_from_secret_key<
        A: Clone + Sync + Send + Serializable + Deserializable,
        R: Rng + CryptoRng + ?Sized,
        D: DB,
    >(
        rng: &mut R,
        ...
        let inp = Input {
            ...
            contract_address: match sk {
                SenderEvidence::Contract(addr) => Some(addr),
                _ => None,
            },
            merkle_tree_root,
            proof: proof_preimage,
    ...
    pub(crate) fn new_for_recipient<R: Rng + CryptoRng + ?Sized>(
        ...
        let outp = Output {
            ...
            contract_address: match recipient {
                Recipient::Contract(addr) => Some(addr),
                _ => None,
            },
            ciphertext,
            proof: proof_preimage,
...
impl Transient<ProofPreimage> {
    ...
    pub fn new_from_contract_owned_output<R: Rng + CryptoRng + ?Sized>(
        ...
        let tree = MerkleTree::<(), InMemoryDB>::blank(ZSWAP_TREE_HEIGHT).update_hash(
            0,
            output.coin_com.0,
            (),
        );
        ...
        let input = Input::new_contract_owned(rng, coin, segment, addr, &tree)?;
        let io = Transient {
...
mod tests {
    use coin_structure::{
        coin::{Info as CoinInfo, NATIVE_TOKEN, PublicKey as CoinPublicKey},
    ...
    fn bad_transient() {
        ...
        let pk = CoinPublicKey(rng.gen());
        let out = Output::new::<_>(&mut rng, &coin, 0, &pk, None).unwrap();
