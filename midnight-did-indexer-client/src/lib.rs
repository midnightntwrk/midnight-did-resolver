...
impl MidnightIndexerClient {
    ...
    pub async fn get_contract_state(&self, did: &MidnightDid) -> Result<ContractState, Error> {
        ...
        let data = response_body.data.ok_or(Error::MissingDataFields {
            url: self.url.to_string(),
            address: address.clone(),
            fields: vec!["data"],
        })?;
        let contract = data.contract_action.ok_or(Error::MissingDataFields {
            url: self.url.to_string(),
            address,
